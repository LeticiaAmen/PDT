		...
		try {
			...
			for (Accion accion : acciones) {
				if(accion.getReclamo().getIdReclamo() == reclamo.getIdReclamo()){
					...
					Object[] rowData = { accion.getFechaHora(), accion.getDetalle(),
							...
							accion.getAnalista().getUsuario().getNombres() + " " + accion.getAnalista().getUsuario().getApellidos()};
			...
			btnRegistrar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					int dialogResult = JOptionPane.showOptionDialog(null, "¿Desea registrar la acción realizada?","Advertencia",
					...
					accion.setReclamo(reclamo);
					 ...
					 String detalle1 = textArea.getText();
				        ...
				        if (detalle1.length() < 3 || detalle1.length() > 255) {
				            JOptionPane.showMessageDialog(null, "La acción debe contener entre 3 y 255 caracteres.", "Error", JOptionPane.ERROR_MESSAGE);
					...
					for (Analista a : analistas) {
						...
						try {
							...
							JOptionPane.showMessageDialog(null, "Acción registrada. Se envió un mail al estudiante.", "Mensaje", JOptionPane.INFORMATION_MESSAGE);
